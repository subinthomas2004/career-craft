...
const Quiz = () => {
  ...
  useEffect(() => {
    ...
    const handleVisibilityChange = () => {
      if (document.hidden) {
        // Immediate termination for tab switching/minimizing
    ...
    const handleViolation = (msg: string) => {
      const now = Date.now();
      if (now - lastViolationTime.current < 2000) return; // Prevent double counting within 2 seconds
      lastViolationTime.current = now;

      setExamWarnings(prev => {
        const newCount = prev + 1;
        console.log(`Violation: ${msg}. Count: ${newCount}`);
  ...
  // Effect to handle termination based on warnings
  useEffect(() => {
    if (examWarnings >= 2) {
  ...
  useEffect(() => {
    if (stage === 'result' && isExamMode) {
      const calculatedScore = (correctCount * 1) - (wrongCount * 1);
  ...
  const handleNext = () => {
    // In exam mode, allow skipping (selectedAnswer can be null)
    // In normal mode, require answer? Original code required answer.
    // Let's keep original behavior for normal mode, but exam allows skipping?
    // User said: "Green: answers, Grey: not attended". Implies navigation or skipping.
    // For simplicity in this iteration, we'll auto-save answer even if null (skip) for exam?
    // Actually, if selectedAnswer is null, it's a skip.

    if (isNavigating || (!isExamMode && selectedAnswer === null)) return;

    const currentQ = quizQuestions[currentQuestionIndex];

    // Remove existing answer for this question if any (to allow re-answering in exam navigation if implemented, 
    // but current flow is linear 'Next'. We'll stick to linear for now unless grid navigation is requested.
    // The request asked for "Right side 50 boxes... Green/Grey".
    // This implies random access navigation.
    // To support random access, we need to update answers map, not just push.

    if (selectedAnswer !== null) {
      setAnswers(prev => {
        // Filter out previous answer for this question
    ...
    if (currentQuestionIndex < quizQuestions.length - 1) {
      ...
      // Check if next question was already answered to restore selection
              ...
              <div className="p-4 rounded-xl bg-red-50 border border-red-100">
                ...
                <ul className="text-sm text-red-600 space-y-2">
                  <li className="flex items-start gap-2">
                    ...
                    <span>Switching tabs or minimizing results in <strong>IMMEDIATE TERMINATION</strong>.</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="mt-1 w-1.5 h-1.5 rounded-full bg-red-400 shrink-0" />
                    <span>Exiting Full Screen allows <strong>1 Warning</strong>. 2nd violation terminates exam.</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="mt-1 w-1.5 h-1.5 rounded-full bg-red-400 shrink-0" />
                    <span><strong>2 Violations</strong> will result in immediate termination of the exam.</span>
                  </li>
                </ul>
              </div>
  ...
  if (stage === "quiz") {
    ...
    return (
      <div className="p-4 sm:p-6 lg:p-8 min-h-screen bg-gray-50 text-gray-900 flex flex-col items-center">
        <div className={cn("w-full transition-all duration-300", isExamMode ? "max-w-[1400px] grid lg:grid-cols-[1fr_300px] gap-6" : "max-w-3xl")}>
          ...
          <div className="space-y-6">
            ...
            {!isExamMode && (
              <div className="space-y-2">
                ...
                <Progress value={progress} className="h-2 bg-gray-200" indicatorClassName="bg-primary" />
              </div>
            )}
            ...
            <Card className="border-gray-200 bg-white shadow-xl overflow-hidden h-fit">
              ...
              <div className="p-6 pt-0 flex justify-end">
                <Button
                  onClick={handleNext}
                  size="lg"
                  disabled={isNavigating || (!isExamMode && selectedAnswer === null)} // In normal mode, force answer. Prevent double-click.
                  className="px-8 font-bold gap-2"
                >
                  {isExamMode ? (currentQuestionIndex < quizQuestions.length - 1 ? "Next" : "Finish") : (currentQuestionIndex < quizQuestions.length - 1 ? "Next Question" : "Submit Quiz")}
          ...
          {/* Right Column: Exam Palette (Only in Exam Mode) */}
          {isExamMode && (
  ...
  if (stage === "result") {
    ...
    // Exam Scoring: +1 Correct, -1 Wrong, 0 Unattempted
    const totalScore = isExamMode ? (correctCount * 1) - (wrongCount * 1) : correctCount;

    // Submit Score if Exam Mode and not already submitted/shown
    if (isExamMode && !showExamResults) {
      // We only want to submit once. The result view logic is a bit complex here because of the 'showExamResults' toggle.
      // Ideally we submit when the exam ENDS (timer/submit), not during render.
      // For simplicity in this existing structure, we can submit in a useEffect when stage changes to 'result'.
    }
    // Normal mode used % in original code.
      ...
      <div className="p-6 lg:p-8 min-h-screen bg-gray-50 text-gray-900 flex flex-col items-center">
        <div className="w-full max-w-4xl">
          ...
          <div className="text-center mb-10 animate-in zoom-in fade-in duration-500">
            ...
            <p className="text-gray-500">
              {isExamMode
                ? <span>Total Score: <strong className="text-primary">{totalScore}</strong> / {quizQuestions.length}</span>
                : <span>You scored {percentage}% in {subject?.name}</span>
              }
            </p>
          </div>

          <div className="grid grid-cols-3 gap-4 mb-8 max-w-2xl mx-auto">
            <Card className="bg-white border-gray-200 shadow-sm p-6 text-center">
              <div className="text-4xl font-bold text-emerald-500 mb-1">{correctCount}</div>
              <div className="text-sm text-gray-500 font-medium uppercase tracking-wider">Correct</div>
            </Card>
            <Card className="bg-white border-gray-200 shadow-sm p-6 text-center">
              <div className="text-4xl font-bold text-red-500 mb-1">{wrongCount}</div>
              <div className="text-sm text-gray-500 font-medium uppercase tracking-wider">Wrong</div>
            </Card>
            <Card className="bg-white border-gray-200 shadow-sm p-6 text-center">
              <div className="text-4xl font-bold text-gray-400 mb-1">{unattemptedCount}</div>
              <div className="text-sm text-gray-500 font-medium uppercase tracking-wider">Skipped</div>
            </Card>
          </div>
